...
module('FlashMessagesService', function(hooks) {
  ...
  test('#queue returns an array of flash messages', function(assert) {
    ...
    assert.equal(this.service.queue.length, 3, 'it returns the correct number of flash messages');
  });
  ...
  test('#arrangedQueue returns an array of flash messages, sorted by priority', function(assert) {
    ...
    assert.equal(typeOf(this.service.queue), 'array', 'it returns an array');
    assert.equal(this.service.queue.length, 3, 'it returns the correct number of flash messages');
    assert.equal(this.service.arrangedQueue[0].priority, 300, 'it returns flash messages in the right order');
    assert.equal(this.service.arrangedQueue[2].priority, 100, 'it returns flash messages in the right order');
  });
  ...
  test('#arrangedQueue is read only', function(assert) {
    ...
    assert.equal(this.service.arrangedQueue.length, 0, 'it did not set #arrangedQueue');
  });
  ...
  test('#add adds a custom message', function(assert) {
    ...
    assert.equal(this.service.queue[0].timeout, 1, 'it has the correct timeout');
    assert.equal(this.service.queue[0].sticky, true, 'it has the correct sticky');
    assert.equal(this.service.queue[0].showProgress, true, 'it has the correct show progress');
  });
  ...
  test('it adds duplicate messages to the queue if preventDuplicates is `false`', function(assert) {
    ...
    assert.equal(this.service.queue.length, 3, 'it adds duplicate messages to the queue');
  });
  ...
  test('it does not add duplicate messages to the queue if preventDuplicates is `true`', function(assert) {
    ...
    assert.equal(this.service.queue.length, 2, 'it does not add duplicate messages to the queue');
  });
  ...
  test('it does use the global preventDuplicates if the instance option isn\'t set', function(assert) {
    ...
    assert.equal(this.service.queue.length, 5, 'it handles duplicates where preventDuplicates is false');
  });
  ...
  test('it supports chaining', function(assert) {
    ...
    assert.equal(this.service.queue.firstObject.message, 'foo', 'should support chaining');
    assert.equal(this.service.queue.lastObject.message, 'bar', 'should support chaining');
  });
  ...
  test('it returns flash object when fetched through `getFlashObject`', function(assert) {
    ...
    assert.equal(flash.message, 'foo', 'it returns flash object with correct message');
    assert.equal(flash.timeout, flashMessageDefaults.timeout, 'it returns an object with defaults');
  });

  test('it supports public API methods for `peekLast` and `peekFirst`', function(assert) {
    this.service.clearMessages();

    assert.equal(typeOf(this.service.peekLast()), 'undefined', 'returns undefined when queue is empty');
    assert.equal(typeOf(this.service.peekFirst()), 'undefined', 'returns undefined when queue is empty');

    this.service.add({ message: 'foo' }).add({ message: 'bar' });

    assert.equal(this.service.peekFirst().message, 'foo', 'returns first object from queue');
    assert.equal(this.service.peekLast().message, 'bar', 'returns last object from queue');
  });
});
